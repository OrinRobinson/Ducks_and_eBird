...
counts_R4 <- sub_18_4 %>%
  ...
  group_by(st_week)%>%
  summarise(NOPI = sum(NOPI))%>%
  mutate(pct = NOPI/max(NOPI), Region = "4")
...
Co_1 <- ggplot(data = IL_surv[which(IL_surv$Region == 1 & IL_surv$Duck_week < 36),], aes(Duck_week, pct)) +
  ...
  geom_line(aes(Duck_week, NOPI_pct, color = "eBird S&T"), eBird[which(eBird$Region==1 & eBird$Duck_week <36),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("NOPI Region 1")+
  theme_light()+
...
Co_2 <- ggplot(data = IL_surv[which(IL_surv$Region == 2 & IL_surv$Duck_week < 36),], aes(Duck_week, pct)) +
  geom_line(aes(color = "IL Survey"))+
  geom_rug(alpha=0.5, size=1.5, sides = "b", show.legend = TRUE, aes(color = "IL Survey Date"))+
  geom_line(aes(Duck_week, NOPI_pct, color = "eBird S&T"), eBird[which(eBird$Region==2 & eBird$Duck_week <36),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("NOPI Region 2")+
  theme_light()+
  theme(legend.title = element_blank())
...
Co_3 <- ggplot(data = IL_surv[which(IL_surv$Region == 3 & IL_surv$Duck_week < 36),], aes(Duck_week, pct)) +
  geom_line(aes(color = "IL Survey"))+
  geom_rug(alpha=0.5, size=1.5, sides = "b", show.legend = TRUE, aes(color = "IL Survey Date"))+
  geom_line(aes(Duck_week, NOPI_pct, color = "eBird S&T"), eBird[which(eBird$Region==3 & eBird$Duck_week <36),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("NOPI Region 3")+
  theme_light()+
  theme(legend.title = element_blank())
...
Co_4 <- ggplot(data = IL_surv[which(IL_surv$Region == 4 & IL_surv$Duck_week < 36),], aes(Duck_week, pct)) +
  geom_line(aes(color = "IL Survey"))+
  geom_rug(alpha=0.5, size=1.5, sides = "b", show.legend = TRUE, aes(color = "IL Survey Date"))+
  geom_line(aes(Duck_week, NOPI_pct, color = "eBird S&T"), eBird[which(eBird$Region==4 & eBird$Duck_week <36),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("NOPI Region 4")+
  theme_light()+
  theme(legend.title = element_blank())
