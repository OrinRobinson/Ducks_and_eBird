...
sp_path <- ebirdst_download(species = "mallar3", force= TRUE)
...
MALL_eBird_R1 <-data.frame(week = date_to_st_week(parse_raster_dates(abd_crop)),
...
saveRDS(MALL_eBird_R1, "MALL_ILL_1.rds")
...
# could add additional masking here, e.g. if you had a county boundary
# mean cell value for each week within extent
MALL_eBird_R2 <-data.frame(week = date_to_st_week(parse_raster_dates(abd_crop)),
                           abd = cellStats(abd_crop, mean),
                           lower = cellStats(low_crop, mean),
                           upper = cellStats(upp_crop, mean),
                           row.names = NULL)
...
saveRDS(MALL_eBird_R2, "MALL_ILL_2.rds")
...
# could add additional masking here, e.g. if you had a county boundary
# mean cell value for each week within extent
MALL_eBird_R3 <-data.frame(week = date_to_st_week(parse_raster_dates(abd_crop)),
                           abd = cellStats(abd_crop, mean),
                           lower = cellStats(low_crop, mean),
                           upper = cellStats(upp_crop, mean),
                           row.names = NULL)
...
saveRDS(MALL_eBird_R3, "MALL_ILL_3.rds")
...
MALL_eBird_R4_a <-data.frame(week = date_to_st_week(parse_raster_dates(abd_crop)),
                             abd = cellStats(abd_crop, mean),
                             lower = cellStats(low_crop, mean),
                             upper = cellStats(upp_crop, mean),
                             row.names = NULL)
...
MALL_eBird_R4_b <-data.frame(week = date_to_st_week(parse_raster_dates(abd_crop)),
                             abd = cellStats(abd_crop, mean),
                             lower = cellStats(low_crop, mean),
                             upper = cellStats(upp_crop, mean),
                             row.names = NULL)
...
MALL_eBird_R4 <- rbind(MALL_eBird_R4_a, MALL_eBird_R4_b) %>% 
  group_by(week) %>%
...
saveRDS(MALL_eBird_R4, "MALL_ILL_4.rds")
