...
IL_Duck_1 <- read_csv("Data/IL_Ducks_Data.csv", 
  ...
  select(c("YEAR","County", "MALL", "st_week", "LOCATION", "REG", "DAY")) %>%
  filter(REG == 1)
...
IL_Duck_2 <- read_csv("Data/IL_Ducks_Data.csv", 
                      col_types = cols(DAY = col_date(format = "%m/%d/%Y"))) %>%
  mutate(st_week = date_to_st_week(DAY)) %>%
  select(c("YEAR","County", "MALL", "st_week", "LOCATION", "REG")) %>%
  filter(REG == 2)
...
counts_R1 <- sub_18_1 %>%
  ...
  summarise(MALL = sum(MALL)) %>%
  group_by(st_week)%>%
  summarise(MALL = sum(MALL)) %>%
  mutate(pct = MALL/max(MALL),Region = "1")
...
counts_R2 <- sub_18_2 %>%
  ...
  group_by(st_week)%>%
  summarise(MALL = sum(MALL))%>%
  mutate(pct = MALL/max(MALL), Region = "2")
...
counts_R3 <- sub_18_3 %>%
  ...
  group_by(st_week)%>%
  summarise(MALL = sum(MALL))%>%
  mutate(pct = MALL/max(MALL),Region = "3")
...
counts_R4 <- sub_18_4 %>%
  ...
  group_by(st_week)%>%
  summarise(MALL = sum(MALL))%>%
  mutate(pct = MALL/max(MALL), Region = "4")
...
Co_1 <- ggplot(data = IL_surv[which(IL_surv$Region == 1 & IL_surv$Duck_week < 35), ], aes(Duck_week, pct)) +
  geom_line(aes(color = "IL Survey"))+
  geom_rug(alpha=0.5, size=1.5, sides = "b", show.legend = TRUE, aes(color = "IL Survey Date"))+
  geom_line(aes(Duck_week, MALL_pct, color = "eBird S&T"), eBird[which(eBird$Region==1 & eBird$Duck_week < 35),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("MALL Region 1")+
  theme_light()+
  theme(legend.title = element_blank())


Co_2 <- ggplot(data = IL_surv[which(IL_surv$Region == 2 & IL_surv$Duck_week < 35),], aes(Duck_week, pct)) +
  geom_line(aes(color = "IL Survey"))+
  geom_rug(alpha=0.5, size=1.5, sides = "b", show.legend = TRUE, aes(color = "IL Survey Date"))+
  geom_line(aes(Duck_week, MALL_pct, color = "eBird S&T"), eBird[which(eBird$Region==2 & eBird$Duck_week < 35),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("MALL Region 2")+
  theme_light()+
  theme(legend.title = element_blank())


Co_3 <- ggplot(data = IL_surv[which(IL_surv$Region == 3 & IL_surv$Duck_week < 35),], aes(Duck_week, pct)) +
  geom_line(aes(color = "IL Survey"))+
  geom_rug(alpha=0.5, size=1.5, sides = "b", show.legend = TRUE, aes(color = "IL Survey Date"))+
  geom_line(aes(Duck_week, MALL_pct, color = "eBird S&T"), eBird[which(eBird$Region==3 & eBird$Duck_week < 35),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("MALL Region 3")+
  theme_light()+
  theme(legend.title = element_blank())



Co_4 <- ggplot(data = IL_surv[which(IL_surv$Region == 4 & IL_surv$Duck_week < 35),], aes(Duck_week, pct)) +
  geom_line(aes(color = "IL Survey"))+
  geom_rug(alpha=0.5, size=1.5, sides = "b", show.legend = TRUE, aes(color = "IL Survey Date"))+
  geom_line(aes(Duck_week, MALL_pct, color = "eBird S&T"), eBird[which(eBird$Region==4 & eBird$Duck_week < 35),])+
  ylab("% of max abundance") +
  xlab("Week") +
  ggtitle("MALL Region 4")+
  theme_light()+
  theme(legend.title = element_blank())
